defmodule Stats.EventsRepo do
  ...
  def arrow_query(query, opts \\ []) do
    ...
    {adapter_meta, opts} = Ecto.Repo.Supervisor.tuplet(repo, prepare_opts(:all, opts))
    ...
    {query_meta, prepared, _cast_params, dump_params} =
  ...
  def arrow_stream(queryable, opts \\ []) do
    repo = get_dynamic_repo()
    {adapter_meta, opts} = Ecto.Repo.Supervisor.tuplet(repo, prepare_opts(:stream, opts))
